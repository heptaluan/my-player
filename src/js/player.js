...
class APlayer {
    ...
    constructor(options) {
        ...
        if (this.options.audio.length === 0 && document.querySelector(".no-data")) {
    ...
    bindEvents() {
        ...
        this.on('error', () => {
            if (this.list.audios.length > 1) {
                this.notice('当前音频播放失败，将在2秒后播放下一首。');
                skipTime = setTimeout(() => {
                    this.skipForward();
                    if (!this.paused) {
                        this.play();
                ...
                this.notice('当前音频播放失败，请检查文件后重新尝试。');
            }
        });
        ...
        this.on('ended', () => {
            if (this.options.loop === 'none') {
                ...
                } else if (this.options.order === 'one') {
    ...
    setUIPlaying() {
        if (this.paused) {
            ...
            document.querySelector('.aplayer-play-title').title = '暂停'
            setTimeout(() => {
    ...
    setUIPaused() {
        if (!this.paused) {
            ...
            this.template.button.innerHTML = '';
            document.querySelector('.aplayer-play-title').title = '播放'
            setTimeout(() => {
